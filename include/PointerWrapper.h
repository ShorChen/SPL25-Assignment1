...
public:
    ...
    PointerWrapper(PointerWrapper&& other) noexcept : ptr(other.ptr) {
        std::cout << "Move constructor called for: " << ptr << "\n";
        other.ptr = nullptr;
    }
    ...
    T& operator*() const {
        if (!ptr) throw std::runtime_error("Dereferencing null PointerWrapper");
        return *ptr;
    }
     ...
     */
    T* operator->() const {
        if (!ptr) throw std::runtime_error("Accessing null PointerWrapper");
        return ptr;
    }
    ...
    T* get() const {
        if (!ptr) throw std::runtime_error("Calling get() on null PointerWrapper");
        return ptr;
    }
    ...
    void reset(T* new_ptr = nullptr) {
        std::cout << "Reset called for pointer: " << ptr << "\n";
        if (ptr != new_ptr) {
            delete ptr;
            ptr = new_ptr;
        }
